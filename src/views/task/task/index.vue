  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="种类" prop="typeId">
        <el-select v-model="queryParams.typeId" placeholder="请选择种类" clearable @change="handleQuery" style="width: 200px;">
    ...
    <el-table v-loading="loading" :data="taskList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="状态" align="center" prop="status">
        <template v-slot="scope">
          <span>{{ scope.row.status === 0 ? '待完成' : '已完成' }}</span>
        </template>
      </el-table-column>

      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button type="success" icon="check" @click="handleFinish(scope.row)" 
            v-hasPermi="['task:task:edit']">完成任务</el-button>
...
import { listTask, getTask, delTask, addTask, updateTask, finishTask } from "@/api/task/task";
...
function getDefaultEndTime() {
  ...
  return formatDate(tomorrow);  // 返回格式化后的日期
}
